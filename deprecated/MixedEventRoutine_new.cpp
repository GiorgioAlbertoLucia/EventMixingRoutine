...
void SaveBinnedTrees(ROOT::RDataFrame& df, YAML::Node& config) {
    ...
    readYamlVector(config["column_names"], columnNames);
    ...
    readYamlVector(config["centrality_bin_edges"], centrality_value);
    readYamlVector(config["z_vertex_bin_edges"], zvertex_value);
...
TTree* SingleBinMixing(TTree* binTree, const YAML::Node& config) {
    ...
    readYamlVector(config["column_names"], columnNames);
    readYamlVector(config["trackA_column_names"], trackAColumnNames);
    readYamlVector(config["trackB_column_names"], trackBColumnNames);
    readYamlVector(config["shared_column_names"], sharedColumnNames);
    std::map<std::string, std::string> columnTypeDict;
    readYamlMap(config["column_type_dict"], columnTypeDict);
...
void DisplayInitialMessage(const YAML::Node& config) {
    ...
    readYamlVector(config["centrality_bin_edges"], centralityEdges);
    readYamlVector(config["z_vertex_bin_edges"], zvertexEdges);
    ...
    readYamlVector(config["tree_names"], treeNames);
    ...
    readYamlVector(config["trackA_column_names"], trackAColumns);
    readYamlVector(config["trackB_column_names"], trackBColumns);
    readYamlVector(config["shared_column_names"], sharedColumns);
